...
spiral (!(Wheel s ms (!ns) : ws)) ps qs = foldr turn0 (roll s) ns
  where roll (!o)
...
nextSize (Wheel s ms (!ns)) p (!q) = Wheel (s * p) ms' ns'
  where (xs, ns') = span (<= q) (foldr turn0 (roll (p - 1) s) ns)
        ms' = foldr turn0 xs ms
        roll 0 _ = []
        roll t o
          = foldr (turn o) (foldr (turn o) (roll (t - 1) (o + s)) ns) ms
        turn0 (!n) rs = if n `mod` p > 0 then n : rs else rs
        turn o n rs
          = let n' = o + n in if n' `mod` p > 0 then n' : rs else rs
main
  = do [arg] <- getArgs
